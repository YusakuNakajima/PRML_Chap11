...
- サンプル$z^{(l)}$が独立でない場合、実効的なサンプルサイズは見かけよりも小さくなる
...
サンプルはコンピュータアルゴリズムによって生成されるため、実際には疑似乱数となるが、適切なランダム性のテストに合格する必要がある
...
ここで、$p(z) = 1$ である
目標は、$y$ の値が特定の分布 $p(y)$ に従うように関数 $f(z)$ を選択することである
...
したがって、$y = h^{-1}(z)$ となり、目標分布の不定積分の逆関数を使用して均一に分布した乱数を変換する必要がある
...
ここで紹介した変換を用いた手法は、必要な分布の不定積分を計算し、それを逆にする能力に依存する
